...
//分配内存————找到首次满足作业空间的空闲区
bool FirstFit::AllocationMemory(Node& work)
{
    sort(FreeList.begin(), FreeList.end(), LessSortByAddress);      //首先利用低址部分的空闲区
...
//退出内存
bool FirstFit::ExitProgress(string& name)
{
